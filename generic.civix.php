 ...
 */
function _generic_civix_civicrm_config(&$config = NULL) {
...
function _generic_civix_civicrm_managed(&$entities) {
  ...
  foreach ($mgdFiles as $file) {
    ...
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'com.cividesk.sync.generic';
      }
      $entities[] = $e;
    }
  }
...
function _generic_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_generic_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
    ...
    $caseTypes[$name] = array(
      'module' => 'com.cividesk.sync.generic',
      'name' => $name,
      'file' => $file,
    );
  }
 ...
 */
function _generic_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _generic_civix_glob(__DIR__ . '/ang/*.ang.php');
  foreach ($files as $file) {
    $name = preg_replace(':\.ang\.php$:', '', basename($file));
    $module = include $file;
    if (empty($module['ext'])) {
      $module['ext'] = 'com.cividesk.sync.generic';
    }
 ...
 */
function _generic_civix_insert_navigation_menu(&$menu, $path, $item) {
 ...
 */
function _generic_civix_navigationMenu(&$nodes) {
  if (!is_callable(array('CRM_Core_BAO_Navigation', 'fixNavigationMenu'))) {
    _generic_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _generic_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _generic_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _generic_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _generic_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
